...
def main():
    ...
    current_date = datetime.datetime.now().strftime("%m_%Y")
    ...
    reports = [os.path.join(reports_dir, f) for f in os.listdir(reports_dir) if
               f"current.csv" in f or f"current.png" in f]

    # Proceed to send email if there are reports to send
    if reports:
        send_email(reports, current_date)
        print("Current month's reports have been sent.")
    else:
        print("No reports to send for the current month.")
