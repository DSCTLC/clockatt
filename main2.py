...
# Automatically determine the previous month based on the current date
today = datetime.now()
...
data = [entry for entry in data if first_day_of_previous_month <= entry['timestamp'] <= last_day_of_previous_month]

# Determine the month and year from the calculated previous month, formatting month with leading zero
month_year_str = first_day_of_previous_month.strftime("%m_%Y")
...
for employee, entries in grouped_data.items():
    ...
    for date in pd.date_range(start=first_day_of_previous_month, end=last_day_of_previous_month):
...
for employee in unique_employees:
    ...
    ax.axis('tight')
    ax.axis('off')
    title = f"Attendance Report for {employee} ({month_year_str})"
    ...
    png_filename = f'attendance_report_{employee}_{month_year_str}.png'
