...
def start_video():
    ...
    if cap is None:
        ...
        set_button_state(tk.DISABLED)  # Disable other buttons
        show_frame()
...
def capture_frame():
    ...
    if cap and current_employee:
        ...
        if ret:
            ...
            if face_locations:
                photo_path = f"current/employees/{current_employee}/photo_{successful_photos}.jpg"
                ...
                if successful_photos >= 5:
                    ...
                    re_enable_buttons()  # Re-enable other buttons after capturing all photos
                else:
                    # Keep capture button enabled and other buttons disabled if not all photos are captured
                    capture_button.config(state=tk.NORMAL)
                    back_button.config(state=tk.NORMAL)
...
def save_employee(name_var, surname_var, pin_var, role_var, add_window):
    ...
    directory = f"current/employees/{name}_{surname}"
    if not os.path.exists(directory):
        os.makedirs(directory)

    # Update the employee_data to reflect the correct photo path format
    employee_data = {
        "name": name,
        "surname": surname,
        "pin": pin,
        "role": role,
        "photos": [f"/{name}_{surname}/"]  # Correct format for photo paths
    }
    update_file_structure(employee_data)

    current_employee = f"{name}_{surname}"
    add_window.destroy()
    start_video()
    add_button.config(state=tk.NORMAL)
...
def edit_employee():
    def start_edit(employee_var, edit_window):
        ...
        if selected_employee:
            ...
            directory = f"current/employees/{selected_employee}"
...
def delete_employee():
    ...
    def delete_employee_data(employee_name):
        try:
            ...
            directory = f"current/employees/{employee_name}"
            if os.path.exists(directory):
